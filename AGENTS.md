...
- 改动发生在独立 worktree 时，文档链接校验必须在**该 worktree 内**执行：在主工作区跑 `bun run docs:check` 会因被删或改名的文件在那里仍然存在而给出假绿灯，断链直到合并后才暴露。同理，引用被移动文件的文档锚点（含 `docs/spec/`、`docs/testing/` 的行内引用）要与拆分同批修正。
