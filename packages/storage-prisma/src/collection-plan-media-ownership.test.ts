...
describe("CollectionPlan media ownership 迁移 (ADR-0023 决策 2)", () => {
    it("把来源配置里的 media 移交给计划，并从来源配置里删掉", async () => {
        const root = await mkdtemp(join(tmpdir(), "collection-plan-media-"));
        ...
        const legacySchema = await createMigrationWorkspace(root, migrations.slice(0, activationIndex + 1));
        ...
        const fullSchema = await createMigrationWorkspace(root, migrations);
        deployMigrations(databasePath, fullSchema);
