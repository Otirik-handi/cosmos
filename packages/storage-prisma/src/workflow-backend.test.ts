...
import { type TestDatabase, withTestDatabase } from "./index.fixtures.js";
...
describe("PrismaWorkflowBackend", () => {
    for (const testCase of workflowBackendConformanceCases) {
        it(`backend conformance: ${testCase.name}`, async () => {
            await withTestDatabase("workflow-backend-conformance", async (database) => {
                const backend = await createBackend(database);
    ...
    it("round-trips state across Prisma clients and rejects stale revisions", async () => {
        await withTestDatabase("workflow-backend-round-trip", async (database) => {
            const backend = await createBackend(database);
    ...
    it("rejects immutable identity changes and missing runs", async () => {
        await withTestDatabase("workflow-backend-immutable", async (database) => {
            const backend = await createBackend(database);
    ...
    it("fails closed when persisted state JSON or projection is corrupt", async () => {
        await withTestDatabase("workflow-backend-corrupt", async (database) => {
            const backend = await createBackend(database);
    ...
    it("adopts envelope-only runs atomically and conceals them from Kernel reads", async () => {
        await withTestDatabase("workflow-backend-envelope", async (database) => {
            const backend = await createBackend(database);
    ...
    it("emits one terminal Run event per terminal Kernel-state transition", async () => {
        await withTestDatabase("workflow-backend-terminal-event", async (database) => {
            const backend = await createBackend(database);
    ...
    it("projects completed ingest output counts into the product Run", async () => {
        await withTestDatabase("workflow-backend-product-counts", async (database) => {
            const backend = await createBackend(database);
    ...
    it("maps Kernel failure states to the shared run.failed event", async () => {
        await withTestDatabase("workflow-backend-terminal-failure", async (database) => {
            const backend = await createBackend(database);
    ...
    it("maps Kernel cancellations to run.cancelled.v1", async () => {
        await withTestDatabase("workflow-backend-terminal-cancel", async (database) => {
            const backend = await createBackend(database);
...
async function createBackend(database: TestDatabase): Promise<PrismaWorkflowBackend> {
    // Test setup uses the checked-in migration without touching a user's database.
    database.deploy();
    return new PrismaWorkflowBackend(database.openClient());
}
