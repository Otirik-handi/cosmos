...
describe("Source identity migration", () => {
    it("backfills known legacy kinds and preserves enabled state", async () => {
        await withTestDatabase("source-migration-known", async (database) => {
            const legacySchema = await createMigrationWorkspace(database.root, legacyMigrations);
            ...
            const currentSchema = await createMigrationWorkspace(database.root, currentMigrations);
            database.deploy(currentSchema);
    ...
    it("blocks unknown legacy kinds before replacing the source table", async () => {
        await withTestDatabase("source-migration-unknown", async (database) => {
            const legacySchema = await createMigrationWorkspace(database.root, legacyMigrations);
            database.deploy(legacySchema);
            ...
            const currentSchema = await createMigrationWorkspace(database.root, currentMigrations);
            expect(() => database.deploy(currentSchema)).toThrow();

            const unchanged = database.openClient();
