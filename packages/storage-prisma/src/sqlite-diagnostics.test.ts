...
it("stays inert while the switch is off", async () => {
    setDiagnosticsEnv({});
    await withTestDatabase("sqlite-diag-off", async (database) => {
        const file = join(database.root, "diagnostics.jsonl");
...
it("writes one structured JSONL line per record when pointed at a file", async () => {
    await withTestDatabase("sqlite-diag-on", async (database) => {
        const file = join(database.root, "diagnostics.jsonl");
...
it("records a real query through the repository client and pins the pragma facts", async () => {
    await withTestDatabase("sqlite-diag-db", async (database) => {
        database.deploy();
        const file = join(database.root, "diagnostics.jsonl");
        ...
        const client = createPrismaClient(database.root);
        try {
