...
/**
 * 包入口契约：冻结 `@cosmos/transport-http` 的公共导出面。
 *
 * 拆分（继承链拆文件 + 门面）只允许移动实现，不允许增删导出；
 * `entry-surface.txt` 由 `bun run scripts/entry-export-surface.ts` 生成，是允许导出的唯一真相源，
...
describe("transport-http package entry contract", () => {
    it("运行时值导出与冻结的导出面一致", () => {
