...
/**
 * 包入口契约：冻结 `@cosmos/domain` 的公共导出面。
 *
 * 拆分（单体拆文件 + 门面）只允许移动实现，不允许增删导出；`entry-surface.txt` 由
 * `bun run scripts/entry-export-surface.ts packages/domain/src/index.ts --out packages/domain/entry-surface.txt`
...
describe("domain package entry contract", () => {
    it("运行时值导出与冻结的导出面一致", () => {
