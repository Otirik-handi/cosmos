...
- **Windows Node smoke 也变红（第二处遗漏）**：修完 lint 重推后，两个远端的 `Windows Node smoke` job 仍失败——`scripts/smoke-node.ps1` 还在 `POST /sources/{id}/activation-commands`，而这条路由已由 1c-1c-b1 删除（CI 日志：`Cannot POST /api/v1/sources/.../activation-commands` 404）。它与 `scripts/e2e/helpers.ts` 是同一处合同改动，但**漏改了这个脚本**。已按同一口径改成 `PATCH /collection-plans/{planId}`（用创建响应的 `planId`／`planRevisionId`），并补一条「计划确实报告 `enabled`」的断言。本地 `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/smoke-node.ps1` 通过（`durableRunStatus: succeeded`、`feedItems: 3`）。
- **一处本机环境噪音与一条脚本约束**：`smoke-node.ps1` 用 `Start-Process -FilePath bun`，本机 PATH 上的 `bun` 只有 `.ps1`（同发现 2），`Start-Process` 报 `%1 is not a valid Win32 application`，把 `node_modules/bun/bin` 加进 PATH 后正常。另外该脚本无 BOM、原本全 ASCII，本轮最初加的中文注释被 Windows PowerShell 5.1 按 ANSI 解码后破坏了续行（报 `-Method` 不是命令），已改为英文注释——**改这个脚本时不要写非 ASCII 注释**。
- **下一步**：第 4 步 contract 单独排期与授权；`lint:web` 与 `smoke-node.ps1` 这类「改了公共合同但本地门禁跑不到的脚本」应补进后续 Task 的验证矩阵。
