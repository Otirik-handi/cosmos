...
- **合并后在 master 重跑浏览器套件时观察到同一 spec 文件内的间歇失败**（2026-09-15）：`phase2-organization.spec.ts` 共 6 个用例，合并后首跑失败 1 例（拆分场景，症状是拆分前已由 API 确认落库的标签/收藏，在拆分后读不到），随后整文件连跑 3 次：2 次 6/6 通过、1 次失败的是**另一个**用例（证据关系场景 `toBeVisible` 超时）；单跑该文件里的拆分用例 5/5 通过。失败点会漂移、单跑不复现，属共享栈上的时序抖动（该栈 6 个场景各跑一次真实 RSS 录入、共用同一 SQLite；仓库已多次记录 Windows SQLite 并行负载抖动）。**本切片可能加剧了它**：页面现在每个阅读流区块各发一次取数请求，叠加页面级阅读流，单位时间并发读比以前多。
- 未运行：Node 进程 E2E、Windows Node smoke、Docker/Compose、发布部署、真实来源联网验收。
- 该抖动的后续：把这个 spec 的每个场景隔离到独立数据根，或让看板区块的取数复用共享请求/缓存，再观察复现率；结论明确前不要把该套件绿灯当稳定信号。本 Task 的验证结论按「分支内 18/18、合并后 17/18（1 例间歇失败）」如实记录。
