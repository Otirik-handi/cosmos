...
- 生命周期阶段：已合并（`1dbaf90`，2026-09-16）。维护者真人验收通过（分区内拖拽落点、松手无中间态、上移/下移按钮与键盘路径均确认与预期一致）。跨分区拖拽经维护者 2026-09-16 裁定为**不做**（理由见 ADR-0010 决定 7）。
- 连贯目标：编辑模式下可以直接拖动区块在分区内排序；拖拽不是唯一路径，上移/下移按钮与键盘操作都不回退。
- 可观察验收（≤3 条）：
  1. 编辑模式下拖动「来源健康」到「阅读流」上方：服务端返回的新树里两者顺序互换，页面顺序同步；
  2. 跨分区拖动：**不做**（维护者 2026-09-16 裁定，理由见 ADR-0010 决定 7）——分区是「用户的一个关注方面」的语义容器、区块是分区内的内容细分，把区块拖到别的分区会破坏这两层语义，且拖拽高频易误触。编辑模式下每个分区各自是一个拖拽区，从 A 分区发起的拖动看不到 B 分区的落点，拖过去松手弹回原位（从代码结构推断，未实测）；跨分区重新归类不是拖拽的职责。
...
- 拖拽库选 `@dnd-kit/core` + `@dnd-kit/sortable` + `@dnd-kit/utilities`（新增运行时依赖）。理由：该库自带键盘传感器与屏幕阅读器播报；备选的原生 HTML5 拖拽在触屏与键盘上支持极差，自研 Pointer Events 要自己补自动滚动、拖拽预览和无障碍播报。
- 只做 **Block**（分区内），不做 Section 拖拽：对应维护者原话里的痛点，且 `updateSection` 要求同时回传 `title`，两套拖拽上下文会让无障碍与 E2E 面翻倍。服务端能力保留，界面不开放。（2026-09-16 收窄：跨分区拖拽经维护者裁定**不做**，理由见 ADR-0010 决定 7——分区是「关注方面」的语义容器、区块是分区内的内容细分；跨分区重新归类保留区块编辑条的「移到」下拉框作为显式入口，维护者同日裁定保留。）
- 落点解析放纯函数模块 `board-drag.ts` 并配单测；提交落点只使用 dnd-kit 的碰撞结果 `over`（拖动预览用的就是它），**不允许再用指针几何自行二次判定**——第一版那样做导致预览与结果分叉，第二轮已删净（见下方缺陷修复）。
...
- **合并记录（2026-09-16）**：分支 `feat/t14-board-drag-sort`（实现提交 `a270079`）经维护者授权以 `--no-ff` 合入 `master`（合并提交 `1dbaf90`）并推送 `origin`。合并后 `git diff a270079 HEAD` 为空，master 的树与分支上跑过门禁的树逐字节一致；**未在 master 工作区重跑门禁**（主工作区未安装依赖），因此门禁结论以分支上那次实测为准。PROJECT-STATUS 在同一轮收尾提交里更新。worktree 与分支保留，删除需另行授权。
