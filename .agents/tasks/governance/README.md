...
- [`G13-collectors/`](G13-collectors/):P4-2 第五个对象、**包入口**——`plugins/collectors/src/index.ts`(1014 行)拆为 6 个分册(`opencli-runner`/`bilibili-connector`/`bilibili-normalize`/`aihot-connector`/`shared`/`registry`),入口改写成门面(导出清单由冻结快照生成)。**已完成实现与验证**(未 commit):入口 **1014 → 17 行**;最大分册 `bilibili-connector.ts` **248 行**(六个分册全部 ≤400);**导出面 16 → 16 逐字节零 diff**、madge **0 环**、collectors 测试 12/12、全量 **130 文件/737 用例**、全仓 typecheck 0、Node 进程 E2E 6/11;补齐该包此前缺失的 `entry-surface.txt`/`entry-contract.test.ts`/`MODULE.md`;`code-baseline.json` **12 → 11 条**。**三条通用规则**(后续切片默认做法):①**符号归使用者**——把同类常量集中到 registry 会形成 `registry → connector → runner → registry` 环,本轮与 G12 各踩一次;②切片脚本的「补 export」正则要用**多行锚点**(注释块会被一起搬走,单行 `^` 会漏);③第三方导入清单从原 import 块推导,不要推测。
- [`G14-workflow-backend/`](G14-workflow-backend/):P4-2 第六个对象、**运行时关键路径**——`packages/storage-prisma/src/workflow-backend.ts`(896 行)拆为 6 个分册(`workflow-envelope-marker`/`workflow-backend-errors`/`workflow-state-fields`/`workflow-state-codec`/`workflow-state-validation`/`workflow-run-events`),**类保留原文件名**所以包入口零改动,只改 5 处内部引用点。**已完成实现与验证**(未 commit):`workflow-backend.ts` **896 → 360 行**、最大分册 `workflow-state-validation.ts` **199 行**(七个文件全部 ≤400);**包入口导出面 10 → 10 逐字节零 diff**、madge **0 环**、聚焦测试 24/24、全量 **130 文件/737 用例**、全仓 typecheck 0、Node 进程 E2E 6/11;`code-baseline.json` **11 → 10 条**。**新形态**:`codec ↔ validation` 互用低层字段助手会成环,抽一层 `workflow-state-fields` 消环——「符号归使用者」的第三种形态是「多方都用 → 抽到共同低层模块」。
- [`G16-test-timeout-budget/`](G16-test-timeout-budget/) + [`G15-workflow-ingest-test/`](G15-workflow-ingest-test/):**先修前置、再重做拆分**的一对。G15 第一次尝试(871 行测试文件按 `it` 拆成 2 个测试文件 + 装置模块)聚焦跑 4/4 通过,但**全量套件连续两次失败**(131 文件 vs master 130 文件),失败落在两个**已登记的已知抖动**上 → **回滚**。G16 查明根因:`vitest.config.ts` 全局 `testTimeout` 本就是 **60 秒**(带注释说明理由),而该文件自己覆盖成 15/15/15/20 秒;本机实测四条用例 5.7/5.2/4.4/5.4 秒,15 秒预算只剩约 2.6 倍余量。**G16 删掉四处覆盖**后重做拆分,**连续两轮全量全绿(131 文件/737 用例)**。对照:master 6/6 绿 → 拆分+旧预算 **0/2** → 拆分+去预算 **2/2 绿**。**已完成实现与验证**(未 commit):parity **458 行**/media 392/装置 90;用例数仍 4;`docs/spec` 与 `docs/testing` 里指向旧路径与**行范围锚点**的引用同批修正;`code-baseline.json` **10 → 9 条**。**台账**:第 7 条更正(原措辞「考虑给该文件显式 `testTimeout`」方向错了)+ 新增第 8 条(`afterEach` 清理撞 `EBUSY`,独立机制,未查清)。**纪律点**:两次失败都不是断言失败、且失败点在两个不同文件间漂移,若按「重跑就过」结案就会漏掉「多出的并发文件在抬高已知抖动触发率」这个真实代价。
...
**完整口径下当前超红线文件（2026-09-24 G14 后实测 4 个，全部为行数越界；行数降序）**：
...
（另有 2 个文件只因入口行数红线越界：`packages/logging/src/index.ts` 674 行、`plugins/rss/src/index.ts` 621 行。已治理：`packages/domain/src/index.ts`（G09，885 → 12 行）、`packages/application/src/media-acquisition.ts`（G10，918 行 → 5 个分册）、`packages/application/src/workflow-host-runtime.ts`（G11，1205 行 → 6 个分册）、`packages/worker-admin/src/index.ts`（G12，1047 → 14 行）、`plugins/collectors/src/index.ts`（G13，1014 → 17 行）、`packages/storage-prisma/src/workflow-backend.ts`（G14，896 → 360 行 + 6 个分册）、`apps/worker/src/workflow-ingest.test.ts`（G15，871 → 2 个测试文件 + 装置模块）。）

**G 系列状态（2026-09-24 更新）**：G01–G07 已收口；G08 为**机制修复**（行数门禁）已完成；G09–G16 为拆分与配套修复，已实现与验证。红线文件已从 G08 时的 **10 个降到 3 个**（**全部是留待 UI 重做的 Web 文件**——即非 UI 侧已清零），`code-baseline.json` 从 16 条降到 **9 条**。G 系列因 P4-2 重启，**不做非红线文件**。选型时仍应核对 `--json` 的 `lines` 字段——门禁现在会拦新增越界，但基线内文件的存量增长只报 warning。

治理队列：**非 UI 侧已清空**（G09–G15 覆盖 domain 入口、media-acquisition、workflow-host-runtime、worker-admin 入口、collectors 入口、workflow-backend、workflow-ingest 测试）。剩下的 3 个红线文件（`product-fixtures.tsx`、`board-view.tsx`、`story-panel.tsx`）**留到 UI 重做同批**——马上要重写的文件先拆等于白拆。G16 是 G15 的前置修复（去掉过小的测试超时预算），与 G15 同分支。**台账第 8 条（`afterEach` 清理撞 `EBUSY`）仍开放**，机制未查清。
