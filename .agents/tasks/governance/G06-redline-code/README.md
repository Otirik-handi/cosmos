...
- **行数红线未入门禁**：`--check` 只判字节/token，行数越界不拦；按提案 §4.1 完整口径仍有 10 个文件超红线（清单见 [`../README.md`](../README.md)「红线口径提醒」）。把行数阈值并入 `size-governance.py` 是根因修复。
- **390px 溢出断言偏薄弱**：`e2e/browser/ingest.spec.ts:127` 的页面级横向溢出检查跨「切换视口 → 立即测量」两步、对时序敏感。2026-09-14 已**两次**在 CI 失败（G06 合并 run、G07 纯文档推送 run——后者代码与前一次成功 run 完全相同），两次都重跑即过、本地多次全绿，故确认是 flake 而非回归。**建议修法**：把测量改为 `expect.poll` 有界重试（容许布局稳定过程中的瞬时溢出，持续溢出仍失败），并在失败消息里带上最宽的溢出元素；不宜继续以「重跑」应对。
