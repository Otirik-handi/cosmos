...
- **contracts**：`index.ts` 变为 ≤100 行门面（只做导出与模块地图），新增 `packages/contracts/MODULE.md`；包入口导出面零 diff（快照 + 常驻契约测试双重确认）。
...
| 顺序 | 对象 | 大小 / 行数 | 形态与护栏 |
|---|---|---|---|
| 1 | `packages/contracts/src/index.ts`（+同名测试） | 89.0 KB / 2466 行 | 桶 + 单体混合（1 处 `export *` + 298 处 export 语句，含公开 DTO）；单元覆盖 100%（G05 评分表 V=10）；消费面 56 个非测试源文件，靠导出面零 diff 守护 |
...
**进行中**（2026-09-14 开工）。worktree `.worktree/g06-redline-code` + 分支 `refactor/g06-redline-code` 已基于 `master` `ebb9ddb` 建立（维护者批复「批准开工」）。**切片 0–2 完成**：三对象基线与浏览器基线建立；contracts 导出面冻结（432 个：219 值 / 213 类型）、`export *` 清零、`MODULE.md` 落地、`index.test.ts` 按域拆为 6 个测试文件（用例数守恒）。每切片门禁：typecheck 0 / unit 81 文件·515 用例 / property 4 / e2e 4 / 浏览器 17 / build:packages 0 / madge 0 环 / 导出面逐字节零 diff。

**提交状态**：切片 0–2 的代码改动在 worktree 未提交，本目录文档在主工作区未提交。根 `AGENTS.md` 要求 commit 需维护者明确授权，而任务计划要求「每切片独立 commit」——已提请授权；**授权前不开始切片 3**（它会再次改 `index.ts`，届时切片 1/3 无法再分开提交）。
...
| 3 | contracts 单体按聚合拆：schema/类型移入聚合模块，`index.ts` ≤100 行门面 | 导出面零 diff；`index.ts` ≤100 行；madge 不新增环 | todo |
