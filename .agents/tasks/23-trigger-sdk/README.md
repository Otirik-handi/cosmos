...
9. **切片 8 per-operation 配置 schema 与 Bilibili `search`**（Proposal [`connection-login-lifecycle-v1`](../../../docs/proposals/connection-login-lifecycle-v1.md) 决定 3，即该 Proposal 的「切片 2」；EXT-006 的「多 operation 有真实消费者」）：✅ 已完成、验证、合并并推送（2026-09-23，worktree `.worktree/ext-006-login-lifecycle`／分支 `feat/t22-ext-006-login-lifecycle`；切片 2 提交 `a89f693`，验收阶段的两处修复与真实验收入口提交 `81a5e7a`；`--no-ff` 合并 `da5ae84` 并推送，worktree 与任务分支已清理）。范围：`sourceOperationManifestSchema` 增可空 `configurationSchema`（`null` = 沿用定义级那份）；canonical 校验改按 `(sourceDefinitionRef, operationId)` 检索（`getSourceConfigurationSchema(ref, operationId)` + `sourceOperationConfigurationSchemas` 注册表 + `bilibiliSearchSourceConfigSchema`，未登记即回退定义级，所以 `source.rss@1` 与 `bilibili.fetch` 的校验结果不变）；catalog 里 Bilibili 声明 `operationIds: ["fetch", "search"]`，`search` 自带配置 schema（`query` 必填、`limit` 可选）、`discoveryContext: "search"`、`stateStoreNamespace: null`；连接器把执行决定抽成 `planBilibiliSearchExecution`／`planBilibiliFetchExecution`（命令参数、要不要带 profile、结果归类一次算清；搜索匿名可用、`kind = video`、`discoveryChannel = search`、locator mode 为 `search`）；API／transport 透传调用方提交的 `operationId`；Web 在 `operationIds` 多于一个时出操作选择器（默认第一个），字段按所选操作渲染、切换定义或操作都重置配置，保存与探测都提交所选 `operationId`（`fieldPresentation` 补「查询词」）。**没有**数据库迁移，也没有新路由。验证：`bun run typecheck` 全仓 PASS；`bun run test` **124 文件 / 726 用例全绿**（基线 720，+6 为本切片新增）；`bun run lint:web` 0 errors（79 条既有 warning）；**Node 进程 E2E 6 文件 / 11 用例全绿**；组件实验室 E2E **19 passed**（含新增「按所选 operation 渲染字段」1 例）；浏览器产品 E2E **两次整套运行：一次 33 passed，一次 31 passed／2 failed**（失败项是[在册的既有抖动](../../../docs/testing/known-unstable-cases.md) `phase2-organization.spec.ts:103` 与 `ingest.spec.ts:119` 的 `toBeFocused`，同构建单跑通过，未以重跑结案）；含新增「按 manifest 声明的操作建出 Bilibili 搜索计划」1 例；**真实来源验收四条全绿**——`test:real:rss`（真实公网，20 条）、`test:real:entry`（经 Webhook 入口触发，20 条）、**`test:real:bilibili-search`（本切片新增的命令，本机 OpenCLI，20 条真实搜索结果、不带 profile）**、`test:real:bilibili`（同一连接下 hot 20 条 + feed 20 条）；`docs:check` 760 文件 0 失败；size 门禁 PASS；`git diff --check` 干净。契约快照 `packages/contracts/entry-surface.txt` 按脚本重生成（+1 行：`bilibiliSearchSourceConfigSchema`）。未运行：Docker（`test:docker`）、`test:real:aihot`（与切片无关，本轮未跑）、真实 Agent 验收（本片不涉及）。依赖：切片 4a／4b（连接承载登录态与探测声明）。

**切片 8 验收时发现并修掉的两个缺口（都是本片自己的）**：①**公开投影会丢掉第二个 operation 的配置**——API 的 Source `config` 投影原来按 `connectorId` 白名单（bilibili 只留 `mode`/`limit`/`schemaVersion`），`search` 的 `query` 读回来就没了，正是 EXT-006「API 按声明展示配置」要守的那条；改成按该 operation 的 canonical schema 取键（读不到 schema 的历史来源退回只留 `feedUrl`），并补了 API 层用例与真实验收断言。②**浏览器全套件里 `webhook-entry` 的点击被拦截**：根因是固定宽度侧栏里的连接面板不收缩（`min-content` 398px，长授权范围 1015px）而画到计划列表上，挡住计划行的按钮；已给那条 flex/grid 链铺 `min-w-0`，修后两次整套运行都没再出现该失败（其中一次 **33/33 通过**）。修复前的截图与 error-context 在 `.agent/tmp/browser-flake-2026-09-23/`，修复后的量测与截图在 `.agent/tmp/measure-sidebar.mjs`／`.agent/tmp/sidebar-after-1280.png`（诊断脚本与证据均不入库）。**另一次整套运行仍有 2 例失败**（`phase2-organization.spec.ts:103`、`ingest.spec.ts:119` 的 `toBeFocused`），是[在册的既有抖动](../../../docs/testing/known-unstable-cases.md)（第 1 条，2026-09-20 的 CI 记录里就是同一对），与本片改动无因果关系，按仓库规则不以重跑结案。
