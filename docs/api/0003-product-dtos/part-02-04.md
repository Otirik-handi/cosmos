...
> **2026-09-23 实现现状**（Proposal [`connection-login-lifecycle-v1`](../../proposals/connection-login-lifecycle-v1.md)）：
> 落地的连接投影是 `ConnectionInstance`——`id`/`name`/`connectorId`/可空 `account`/可空
> `scopeJson`/可空 `configJson`（适配器**非秘密**配置，Bilibili 的 OpenCLI profile 住在这里）/
> `status`（`active|revoked|expired|error`）/可空 `secretRef`/可空 `lastError`/时间戳；
> **没有** `revisionId`、`referencedBy` 与 `scopes[]`（授权范围是 `scopeJson` 文本）。
> 本页其余字段与 `authorization-sessions`／`revocations` 仍属 Planned，收敛动作见该 Proposal
> 的待裁定项 3。
