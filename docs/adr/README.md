...
- [0018 Trigger / SDK v1](0018-trigger-sdk-v1.md)：固定 `TriggerBinding` 实体（单绑定 schedule/manual）+ 从 `SourceInstance.config.scheduleIntervalMs` 迁出（migration 回填到 schedule TriggerBinding，调度循环改读 `listScheduleTriggers`）、`SourceDefinitionManifest` 扩展 `auth`（none/oauth/cookie/secret_ref/external）+ `operations`（input/output/external key/discovery/media/stateStore 命名空间）、EXT-003 版本化合同保持现状；webhook/事件触发、自定义 Trigger/Action 插件运行时、多计划后置；Phase 2 第十三切片实施顺序与边界。
