...
- 需要用户状态（收藏/标签/收藏夹/批注/Spotlight）的显式迁移、批量与撤销（待决定事项 10）。（2026-09-14 注记：该条件已触发并处理，结论见 ADR [`0020`](0020-story-split-user-state-migration-v1.md)——迁移是独立命令、反向调用即撤销、作用域限定同一 split 家族；本 ADR 决定 4「用户状态留在历史壳，v1 不迁移」的边界相应由 ADR-0020 决定 7 补充为「迁移是历史壳上唯一允许的写操作」。本 ADR 其余决定不变。）
