...
看板配置是单机本地事务 + 审计，不产生外部副作用：API command → Application 命令 → repository 事务 → 领域事件，不新增 Workflow/Job 类型，复用 ADR-0006–0009 的 `actor/幂等键/白名单投影` 模式。后置：自动 Spotlight policy/Trend Signal（Phase 4）、Workspace/Artifact Block（BRD-005，Phase 3）、网格布局编辑器、Board/Query snapshot 与 Publication（Phase 5）、Feed 反馈与 Read State「未读」过滤。（2026-09-16 注记：**拖拽排序不再是后置项**——维护者按 2026-09-15 真人验收结论把「v1 纵向流 + 上移/下移」中的拖拽排序升级为必做，理由是用上下按钮排序让人烦躁、容易丢失注意点。落地形态见决定 7。）

### 7. 区块拖拽排序只改展示顺序（2026-09-16 增补）

区块排序的落地形态：编辑模式下每个区块有独立拖动按钮，用 `@dnd-kit` 的指针与键盘两个传感器；提交落点**只使用 dnd-kit 的碰撞结果 `over`**（拖动期间"其它区块让位"的预览就是按它算的），按 arrayMove 语义翻译成**一次** `moveBlock(blockId, sectionId, position)` 调用。不允许再用指针几何自行二次判定：那会让预览与结果分叉（实测过预览 `[A,C,B,D]`、落库 `[A,C,D,B]`，以及向上拖被算成 no-op）。服务端 `position` 的口径是「先移除被拖区块、再在剩余区块之间插入」，与 arrayMove 等价。松手时先按同一语义在本地落定再等服务端返回覆盖（失败回滚），并关闭浮层的落位回弹，使「预览 → 结果」之间没有中间态。拖拽与既有的上移/下移按钮并存——键盘路径（Space 抓取 + 方向键）和按钮路径都不回退。

**拖拽范围只限分区内，不做跨分区拖拽（维护者 2026-09-16 裁定）**：分区的语义是「用户的一个关注方面」（人工智能、动漫、体育、音乐等，或更具体的 gpt/deepseek），由用户按自己的偏好组织；区块是**分区内**对内容的进一步细分（热点、精华等）。看板 → 分区 → 区块因此是「偏好 → 关注方面 → 内容细分」的三层语义，把区块从一个分区拖到另一个分区会同时破坏分区与区块两层的含义。拖拽又是高频、容易误触的动作，误把区块搬到别的关注方面代价高，所以跨分区**不是拖拽的职责**：编辑模式下每个分区各自是一个拖拽区，拖到别的分区松手就是无效操作（弹回原位）。这条是产品语义决定，不是待办。**跨分区重新归类保留显式入口**（区块编辑条的「移到」下拉框，维护者 2026-09-16 裁定保留）：它是用户有意的动作、误触代价低，与拖拽的取舍不同；服务端 `moveBlock` 的 `sectionId` 能力与存储层测试都保留。分区自身的排序同样不在范围（服务端 `updateSection` 的 `position` 已支持，界面未开放）。
...
- 多列/网格布局或拖拽编辑器进入产品范围，`position` 字段与移动命令需要重审；（2026-09-16 注记：**拖拽编辑器已落地**——纵向流、仅分区内，见决定 7。该条已被触发并处理：本轮复审了 `position` 语义与移动命令，确认口径为「先移除被拖区块、再在剩余区块之间插入」，并修掉了客户端按全量数组下标传值导致的落点偏移。多列/网格布局仍未进入范围，届时需再审。）
