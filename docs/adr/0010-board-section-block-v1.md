...
**拖拽范围只限分区内，不做跨分区拖拽（维护者 2026-09-15 裁定）**：分区的语义是「用户的一个关注方面」（人工智能、动漫、体育、音乐等，或更具体的 gpt/deepseek），由用户按自己的偏好组织；区块是**分区内**对内容的进一步细分（热点、精华等）。看板 → 分区 → 区块因此是「偏好 → 关注方面 → 内容细分」的三层语义，把区块从一个分区拖到另一个分区会同时破坏分区与区块两层的含义。拖拽又是高频、容易误触的动作，误把区块搬到别的关注方面代价高，所以跨分区**不是拖拽的职责**：编辑模式下每个分区各自是一个拖拽区，拖到别的分区松手就是无效操作（弹回原位）。这条是产品语义决定，不是待办。**跨分区重新归类保留显式入口**（区块编辑条的「移到」下拉框，维护者 2026-09-15 裁定保留）：它是用户有意的动作、误触代价低，与拖拽的取舍不同；服务端 `moveBlock` 的 `sectionId` 能力与存储层测试都保留。分区自身的排序同样不在范围（服务端 `updateSection` 的 `position` 已支持，界面未开放）。
...
- 多列/网格布局或拖拽编辑器进入产品范围，`position` 字段与移动命令需要重审；（2026-09-15 注记：**拖拽编辑器已落地**——纵向流、仅分区内，见决定 7。该条已被触发并处理：本轮复审了 `position` 语义与移动命令，确认口径为「先移除被拖区块、再在剩余区块之间插入」，并修掉了客户端按全量数组下标传值导致的落点偏移。多列/网格布局仍未进入范围，届时需再审。）
