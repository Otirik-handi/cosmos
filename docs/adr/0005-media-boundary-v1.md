...
- 进入 ING-009：per-source 媒体策略、失败重试或历史回填需要改变预算/状态/生命周期语义；（2026-09-09 更新：[`0014`](0014-per-source-media-policy-v1.md) 已落地来源级图片开关与预算收紧；[`0015`](0015-media-retry-retention-v1.md) 已落地失败重试与保留期清理——决策 7「不自愈」被限定为「条目修订不变时不重试，但来源的下一次采集会按上限重试可恢复的失败媒体」，公共 4 态与 `errorMessage` 合同不变，历史媒体回填仍是本条的未决部分。）
