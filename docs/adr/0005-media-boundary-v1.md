...
- 进入 ING-009：per-source 媒体策略、失败重试或历史回填需要改变预算/状态/生命周期语义；（2026-09-09 更新：[`0014`](0014-per-source-media-policy-v1.md) 已落地 ING-009 的 v1 部分——来源级图片开关与预算收紧，预算/状态/生命周期语义未变；失败重试与历史回填仍是本条的未决部分。）
