...
2026-09-09 起草 Proposal 并列出三项裁决（v1 范围、预算方向、策略存放位置）；维护者评审问题未获回复，Agent 按推荐默认推进实施，本 ADR 记录该默认，维护者可随时否决并要求返工。
...
- 失败重试或历史回填进入实施，需要定义改写已存 Asset 的写路径；
- 保留期/清理任务进入实施，需要定义删除确认、引用检查与孤儿 Blob 回收；
