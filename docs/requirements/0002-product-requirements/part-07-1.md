...
**AUT-009 连接可见性注记（2026-09-23）**：AUT-009 记为 **Phase 2 已交付**——可创建可复用连接、多个来源／计划引用同一连接（Task 22 与采集计划 v1）、撤销凭证不删历史、普通配置与日志不含凭证明文此前已交付；验收条件里的「用户能看到连接状态、授权范围和失效原因」由 Task [`22`](../../../.agents/tasks/22-connection-state-store/README.md) 追加切片 3 补齐：连接面板显示授权范围（JSON 顶层标量对象按 `键: 值` 渲染）与失效原因，建连接时可记录授权范围（要求合法 JSON），失效时用户可填原因并标记、恢复后清除。这两个字段今天**没有自动写入方**（真实认证 Adapter 属 EXT-006 的后续），面板提供的是用户侧记录入口；决定与验证见该 Task 记录。
