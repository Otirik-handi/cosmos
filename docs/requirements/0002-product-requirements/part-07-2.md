...
**ING-012 收口注记（2026-09-23，[`connector-state-export-v1` Proposal](../../proposals/connector-state-export-v1.md) accepted）**：ING-012 记为 **Phase 2 已交付**——验收条件里「状态可备份、恢复、迁移并按 Connection／Source 范围隔离」由本片补齐：新增 `ConnectorStateNamespace` 归属表（只存 `planId`，来源／连接靠 join 采集计划取得），宿主在解析状态句柄时登记一次，`putState` 与 CAS 路径不变；`GET /connector-state/namespaces` 给出抽屉清单与归属，`GET /exports/connector-state` 按命名空间／计划／连接／来源四选一导出（缺省 = 全部已归属，未归属抽屉只能点名），`POST /imports/connector-state` 默认只补缺失、显式 `overwrite` 时 `version = 本地 + 1`，单抽屉可改名且目标抽屉必须已登记；Web 存储面板提供导出下载与导入上传。导出件只含 `ConnectorState`，不含 Secret、连接配置、`Checkpoint` 或其它表；整库副本仍由 `POST /backups` 承担，用户数据导出仍不含 `ConnectorState`。决定见 [ADR-0026](../../adr/0026-connector-state-export-v1.md)。上述注记只排定实现顺序，不改变本表最终验收条件。
