...
- **有效 V 以「主要执行层被数据源覆盖」为前提**:若对象主要经由浏览器用例或端到端路径执行(UI 组件、component-lab fixture 等),单元覆盖率**不构成有效 V**——即使单元源报告了非零数字(部分导入产生的低覆盖),也一律记 `V` 未测。理由:G03 复核时 `product-fixtures.tsx` 单元覆盖率 23.21%(V=2)会把 `P` 抬到 5.18、反超 `app.controller.ts`(2.96),而它的真实消费路径在 component-lab 与浏览器用例,该低覆盖只是「部分被单元导入」的产物,不是质量信号。
- **为什么需要这两条**:G03 重排时 `page.tsx`、`story-panel.tsx`、`board-view.tsx` 的单元覆盖率为 0(V 未测),按 0 代入使 `V + R = 0` 且 `P` 趋于无穷,把三个 UI 对象误顶到榜首;而 `product-fixtures.tsx` 属于同一族的「低而非零」变体。两者的真实覆盖都来自浏览器用例,不在该数据源内。
...
| 2026-09-14 | **V 口径修订并并入 §4.2**(依据 G03 重排实测):①分母改为 `max(V + R, 1)` 消除除零;②V 绑定单元覆盖率数据源,换源即换口径;③`V = 0` 为「补测试前置」门禁,不作为代入数值;④`V` 未测(如由浏览器用例覆盖的 UI 对象)不得按 0 代入,不参与 P 排序、单独按 `N = T+L+H+D` 排序并标注;⑤有效 V 以「主要执行层被数据源覆盖」为前提,UI 组件与 component-lab fixture 即使报告非零低覆盖也一律记未测。起因:G03 重排中三个 UI 对象单元覆盖率为 0、`product-fixtures.tsx` 为 23.21%(低而非零),分别把 `P` 推向无穷与 5.18,均误超 `app.controller.ts`(2.96)。按修订后口径回算 G03 候选:`app.controller.ts` 2.96 居首(与已验收的 G03 选型一致),其后 `transport-http/index.ts` 2.31、`application/index.ts` 1.84、`contracts/index.ts` 1.27。附带事实:`@vitest/coverage-v8` 已于 G03 切片 1 落地(§4.7 阶段③部分完成) | 用户(指示处理)+ Agent(并入) |
