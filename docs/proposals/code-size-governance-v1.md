...
2. 建立代码侧阈值体系、拆分模式库、MODULE.md 索引规范、Agent 阅读协议、SOP 与 CI 门禁(只拦未登记文件进入警戒区或红线;基线内文件增长只报 warning,不阻塞)。
...
| CI 门禁 = 基线只减不增 | 修订(2026-09-14,与文档侧同批):存量豁免、新增即拦;基线条目只许删不许加、登记值只许下调(治理目标),基线内文件增长只报 warning 不阻塞 |
...
| 2026-09-14 | **CI 门禁口径修订并并入 §2/§5**(与文档侧 [`oversized-doc-splitting-v1.md`](oversized-doc-splitting-v1.md) §4.10 同批):基线内文件的增长由 fail 改为 warning,CI 只对「未登记文件进入警戒区或红线」fail;登记值与「条目只许删不许加」保留为治理目标。起因:登记值等于登记当天尺寸,零余量下对存量文件的任何正当编辑都阻塞 CI(文档侧 2026-09-13 连续 5 次推送因此失败)。代码侧当前无越界条目,但共用同一脚本与判据,故与文档侧同批修订以保持一致 | 用户(指示处理)+ Agent(并入) |
