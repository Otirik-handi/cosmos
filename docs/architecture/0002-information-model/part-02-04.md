...
**v1 切片（[`entry-duplicate-relations-v1` Proposal](../../proposals/entry-duplicate-relations-v1.md)，2026-09-16 accepted，ADR [`0022`](../../adr/0022-entry-duplicate-relations-v1.md)）**：本节三个关系词按原样落地，不新增也不改名——`duplicate_of` 与 `near_duplicate_of` 对称（存储按条目 id 字典序归一化、双向读取），`syndicated_from` 有向（转载方 → 原发方）；一张 `EntryRelation` 表、一对条目一个当前语义、自关联与反向重复提交返回 409。关系挂在**条目内容身份**上，Story merge/split 不迁移也不修改，条目删除时级联。v1 **只人工标记**（本节列出的候选信号与自动判定属 ORG-021），并且**只做标记与展示**：不参与 Feed 排序、搜索与去重，也不折叠来源身份（属 Phase 4 推荐体系）。
...
**v1 切片（[`story-key-facts-and-time-range-v1` Proposal](../../proposals/story-key-facts-and-time-range-v1.md)，2026-09-16 accepted，ADR [`0021`](../../adr/0021-story-key-facts-and-time-range-v1.md)）**：本节的最小字段集从 title/summary/kind/subtype 扩为四项——新增「时间范围」（复用既有时间语义的起止 + 精度 + 依据，**不按 kind 限制、可留空**）与「关键事实」（有序清单 ≤20 条，每条挂一条出处，**出处不要求属于本 Story**）。两项都参与确定性 fingerprint 判定，且**为空时指纹与升级前逐字节相同**——既保证无实质变化的更新仍是 no-op，也避免升级时为既有数据回填指纹；写入沿用「全量提交当前表示」语义（省略即清空），split 后继同批支持但不迁移原值。「结构化概览」、字段级 producer/version、事实/推断标记与字段级保护（ORG-019）后置。
