...
除镜像构建定义外，Compose 行为是当前配置意图；本基线尚未在当前机器上实际启动容器验证。
...
- 当前基线没有容器运行验证，因此不能把已声明的健康门槛、卷权限和容器内文件布局视为已通过实机验收。
...
- 给定基线 `5ce628690ab0110b0525e8ebcbacbe673ced9c55` 和完整锁文件 → 观察 `oven/bun:1.3.14` 阶段完成冻结依赖安装及工作区构建 → 且不发生锁文件更新或未锁定依赖安装。
