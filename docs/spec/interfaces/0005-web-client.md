...
- **人工已修改标记**：`StoryDetail.story.producer` 为 `human` 时，事件时间行下方显示「人工已修改 ·
  自动更新已暂停，来源的新版本不会覆盖这里的内容」（`data-story-human-protected`）。这条标记说明
  的是**为什么这个 Story 不再跟随来源**，不是可操作控件：解除保护需要新命令，ADR-0028 不提供。
...
22. **存储面板（OPS-003/004）**：侧栏“存储”区的 `StoragePanel` 挂载时并行 `client.storageStats()`、
    ...
    序列化成 JSON 文件交给浏览器下载（文件名 `cosmos-user-data-<exportedAt>.json`，与 API 的
    `Content-Disposition` 用同一约定），并显示各类对象的条数。导出只读、不落盘，内容不含采集内容、
    连接配置与 Secret（ADR-0019 决策 5）。
    面板另有「连接器状态」一组（ING-012 / ADR-0026）：范围下拉默认「全部已归属」，其余选项按名字列出
    抽屉并标出「未归属」（未归属抽屉只能点名导出）；「导出连接器状态」调
    `client.exportConnectorState(scope)` 并下载 `cosmos-connector-state-<exportedAt>.json`。「导入连接器状态」
...
- 采集计划看板：无计划显示“创建第一个采集计划。”；**按连接分组**（无连接的计划归
  ...
  enabled 计划另有手动录入按钮，disabled 计划的运行按钮禁用。每个计划行还展示 Webhook 入口
