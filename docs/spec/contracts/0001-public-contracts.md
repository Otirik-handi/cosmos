...
- **StoryDetail**：`story`（`id`、`kind`、可空 `subtype`、`revisionId`、`title`、可空 `summary`）、`entry: EntryDetail`（最近更新成员，兼容位）以及 `entries: EntryDetail[]`（Story 全部成员，按 Entry `updatedAt` 倒序），并新增 `entities: StoryEntitySummary[]`——每个已关联 Entity 的当前快照（`entityId`、`name`、`type`、producer、可空 `producerVersion`、`confidence`、可空 `evidence`/`actor`/`reason`），用于 Story 侧渲染“涉及哪些实体”而不二次查询。用户组织 v1 追加 `labels: LabelRef[]`（已附加到本 Story 的标签 `id`/`name`）与 `favorited: boolean`（本 Story 的轻量收藏标记，ADR-0009）。Entry↔Story 证据关系 v1 追加 `evidence: StoryEvidence[]`（把本 Story 当作证据/提及目标的其它条目：`entryId`、`sourceId`、`sourceName`、读取侧宽松 `relationType`、可空 `title`、producer、可空 `producerVersion`、`confidence`、可空 `evidence`/`actor`/`reason`，按 createdAt 倒序）。**RevisionDetail** 是 EntryRevisionSnapshot 加上 `entryId`、`sourceId`、`sourceName`、`sourceKind`。
