...
- **StoryDetail**：`story`（`id`、`kind`、可空 `subtype`、`revisionId`、`title`、可空 `summary`）、`entry: EntryDetail`（最近更新成员，兼容位）以及 `entries: EntryDetail[]`（Story 全部成员，按 Entry `updatedAt` 倒序），并新增 `entities: StoryEntitySummary[]`——每个已关联 Entity 的当前快照（`entityId`、`name`、`type`、producer、可空 `producerVersion`、`confidence`、可空 `evidence`/`actor`/`reason`），用于 Story 侧渲染“涉及哪些实体”而不二次查询。**RevisionDetail** 是 EntryRevisionSnapshot 加上 `entryId`、`sourceId`、`sourceName`、`sourceKind`。
