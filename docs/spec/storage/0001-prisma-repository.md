...
- `story`、`entry`、`revision` 返回 contracts 规定的白名单投影；`story` 先按 `StoryAlias` 把旧 id 解析到 canonical Story，再返回 `entry`（最近成员，兼容位）、`entries`（全部成员，updatedAt 倒序）与 `entities`（该 Story 已关联 Entity 的当前快照列表，取自 `StoryEntity` + Entity 当前 Revision）。不存在、或 Story/Entry 没有 current revision 时返回 `null`。修订与观察按实现中的时间/修订排序读取。
