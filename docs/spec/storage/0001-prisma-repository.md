...
- `updateStoryRevision` 以 `baseRevisionId` CAS：base 不是当前 Revision 时抛 `StoryRevisionConflictError`；展示字段 fingerprint 与当前相同则 no-op，否则按当前 revision 加 1 追加 Revision、移动 current 并写 `story.revision_created.v1`。历史壳拒绝改 Revision，抛 `StoryRevisionConflictError`（ADR-0012 决策 6）。
