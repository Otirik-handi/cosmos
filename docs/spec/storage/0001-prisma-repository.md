...
| `ConnectionInstance` | id、name、connectorId、可空 account、可空 scopeJson（授权范围）、可空 configJson（适配器**非秘密**配置，JSON 文本）、status（`active`/`revoked`/`expired`/`error`）、可空 secretRef、可空 lastError、时间戳 | 可复用连接身份（ADR-0017）；`secretRef` 只指向 SecretStore 里的字节，凭证不进本表；`configJson` 装适配器自己的非秘密配置（Bilibili 的 OpenCLI profile，Proposal connection-login-lifecycle-v1），连接器从入队冻结的连接投影读它；计划删除 SET NULL、来源的 `connectionId` 由仓库事务显式置空 |
