...
- `story`、`entry`、`revision` 返回 contracts 规定的白名单投影；`story` 先按 `StoryAlias` 把旧 id 解析到 canonical Story，再返回 `entry`（最近成员，兼容位）、`entries`（全部成员，updatedAt 倒序）、`entities`（该 Story 已关联 Entity 的当前快照列表，取自 `StoryEntity` + Entity 当前 Revision）、用户组织投影 `labels`（`LabelAssignment` + Label name，ADR-0009）和 `favorited`（Story 级 `Favorite` 是否存在），以及证据关系投影 `evidence`（`EntryStoryLink` + 条目来源与当前标题，ADR-0011）。`entries[].relatedStories` 由同一张表按成员 id 一次查询后在内存分组得到。不存在、或 Story/Entry 没有 current revision 时返回 `null`。修订与观察按实现中的时间/修订排序读取。
- `moveEntryToStory` 单事务内把 Entry 主归属改到目标 Story 并写 `story.entry_moved.v1`；Entry 已在目标 Story 时为幂等 no-op，Entry 缺失返回 `null`，目标缺失抛 `StoryNotFoundError`。**Entry↔Story 证据关系存在后**，同一事务内删除该 Entry 指向新主 Story 的冗余关联（有删除时写 `entry.story_unlinked.v1`，payload 带 `cause: primary_story_changed`），保持“不指向自己主 Story”的不变量（ADR-0011 决策 4）。
