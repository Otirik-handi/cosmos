...
| P4 | P4-1 | Phase 2 浏览器用例仍有未归因的间歇失败 | 数据库线已证伪（Task 34）；前端线**复现并修掉一条**：产品缺陷（丢失更新竞态），不是测试抖动；**该条记录里其余失败点仍未归因**——见下 | **部分消除**（1 条已修，其余未归因） |
...
- **现状【实测归因，2026-09-25】**：**只复现并归因了一条**——整套跑一次（1 failed / 32 passed，7.5 分钟）并在失败后立刻拷出 trace，查明 `phase2-organization.spec.ts:30` 是在「给第二条 Story 加已有标签」这一步等满 300 秒超时，因为**标签选择下拉框从未渲染**；它**不是测试抖动，是产品缺陷**（机制见下）。
...
- **仍未归因的部分（别把本条读成「整条已结案」）**：本条记录里另有 `phase2-organization.spec.ts:103`（证据关系反向视图等待超时）、`:417`（用户组织场景耗时异常，单次 5.1 分钟）与 `collection-plan-multi.spec.ts:63`（等「录入任务已排队」提示 15 秒超时）**仍未归因**；`media-policy.spec.ts` 的来源健康行（[`known-unstable-cases.md`](docs/testing/known-unstable-cases.md) 第 4 条）同样未归因，且自称与本条的 `:103`／`:417` 同类。**修后浏览器验收连绿两遍是支持性证据、不是消除证明**（修前 8 轮里 4 轮红，两连绿不足以排除其余成因）。本次修的是一**类**问题（用户可变列表的无守卫写入），它**有可能**解释其余几个「等元素出现却超时」的点，但这是**假设、未验证**，不作为归因。
