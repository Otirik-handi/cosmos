...
/**
 * 包入口契约：冻结 `@cosmos/plugin-collectors` 的公共导出面。
 *
 * 入口拆分（桶文件 → 门面）只允许移动实现，不允许增删导出；`entry-surface.txt` 由
 * `bun run scripts/entry-export-surface.ts plugins/collectors/src/index.ts --out plugins/collectors/entry-surface.txt`
...
describe("collectors package entry contract", () => {
    it("运行时值导出与冻结的导出面一致", () => {
