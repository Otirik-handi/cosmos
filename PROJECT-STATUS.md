...
- **CI 曾卡在 `bun run db:validate`（2026-09-16 诊断，Task [`27`](.agents/tasks/27-prisma-cli-resolution/README.md) 修复，2026-09-17 远端 CI 全绿）**：`bun install --frozen-lockfile` 生成的布局不保证存在 `packages/storage-prisma/node_modules/prisma`（本机全新 worktree 安装即复现：CLI 提升到工作区根、无 `.bun` store），而 27 处引用（含 3 处文档描述）硬编码该路径 → `error: Module not found`；对齐 bun 1.4.2（`4a0a5a3`）无效，驱动因素是 lock（G02 `cc97d4c`、G03 `cb2b082`、Task 14 `a270079` 换 npmmirror 源并丢 `configVersion`）。修复 = `resolvePrismaCliPath()` 按候选位置解析（包内 / 工作区根 / bun store，找不到明确报错）替掉全部 27 处，行为等价、不动锁文件与 `BUN_VERSION`；本地修前红、修后门禁绿，远端 CI（run 35178356761）四个 job 全绿。
