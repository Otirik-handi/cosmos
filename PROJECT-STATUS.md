...
- **CI 现在卡在 `bun run db:validate`（已诊断，未修）**：`bun install --frozen-lockfile` 在 CI（Linux）上不创建 `packages/storage-prisma/node_modules/prisma`，而 `scripts/prisma.ts`、Docker 启动命令、`docs/spec/operations/0002` 与约 20 个 storage 测试**硬编码**了这个 Prisma CLI 路径（共 27 处）→ `error: Module not found`。把 CI 的 bun 对齐到 1.4.2（`4a0a5a3`）**无效**，说明驱动因素是 lock 文件：G02 `cc97d4c`、G03 `cb2b082`、Task 14 `a270079` 把它换成 npmmirror 源并丢了 `configVersion`，而上次 CI 绿（`67ce4b9`）时还是默认源 + 带 `configVersion`。治本方案 = 改为按候选位置解析 Prisma CLI（27 处、机械但面广），**应单独开 Task（编号待分配，26 已给 ING-006）**；维护者 2026-09-16 指示本轮只记录、暂不修改。在此之前 CI 的 typecheck / 全量单测 / property / lint / build / Node E2E / Browser E2E 仍不会执行。
